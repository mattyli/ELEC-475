...
def main():
    ...
    # NOTE: -l parameter added to conform with requirements from the lab document
    ...
    train_transform = transforms.Compose([transforms.ToTensor()])   # transformation pipeline
    ...
    while idx >= 0:
        ...
        if 0 <= idx <= current_set.data.size()[0] and 0 <= idx_2 <= current_set.data.size()[0]:
            ...
            with torch.no_grad():
                reconstruction = model(img) # simple reconstruction of image
                output = model(noisy_img)   # denoising
            ...
            for i, picture in enumerate(interpolated_imgs):
                ax[i+1].imshow(picture, cmap='grey')
